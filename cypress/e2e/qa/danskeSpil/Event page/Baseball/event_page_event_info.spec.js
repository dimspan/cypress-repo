...
describe('Event info tests on event page - Baseball', () => {
  ...
  context('Tests Event info  on event page - Baseball', () => {

    it('C1 - Event info  widget exists - Baseball', () => {
      cy.visit(url);
      po.event_info_exists();
    });
 
    it('C2 - Event info finished events texts - Baseball', () => {
      // cy.visit(url);
      po.check_all_texts_finished_events();
    });

    it('C3 - Event info finished events texts on mobile - Baseball', () => {
      // cy.visit(url);
      cy.viewport(360,1100)
      po.check_all_texts_mobile_finished_events();
    });

    // it('C4 - Event info not started events texts - Baseball', () => {
    //   // cy.visit(url);
