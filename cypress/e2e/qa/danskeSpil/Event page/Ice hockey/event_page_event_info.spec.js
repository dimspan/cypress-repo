...
describe('Event info tests on event page - Ice hockey', () => {
  ...
  context('Tests Event info  on event page - Ice hockey', () => {

    it('C1 - Event info  widget exists - Ice hockey', () => {
      cy.visit(url);
      po.event_info_exists();
    });
 
    it('C2 - Event info finished events texts - Ice hockey', () => {
      // cy.visit(url);
      po.check_all_texts_finished_events();
    });

    it('C3 - Event info finished events texts on mobile - Ice hockey', () => {
      // cy.visit(url);
      cy.viewport(360,1100)
      po.check_all_texts_mobile_finished_events();
    });

    it('C4 - Event info not started events texts - Ice hockey', () => {
      // cy.visit(url);
