...
describe(' Main tournament page and functionalities',()=>{
    ...
    context('Tournament page and tests in PC view',() =>{ 
        ...
        it("C3 - Checks the last matches livescore tp",() =>{
            cy.wait(1000)
            const resizeObserverLoopErrRe = /^[^(ResizeObserver loop limit exceeded)]/
             Cypress.on('uncaught:exception', (err) => {
                /* returning false here prevents Cypress from failing the test */
            ...
            poResults.overview_results_livescore()
            // poResults.tournament_overview_livescore_results_styles_tooltip()
        })
