export class league_page_livescore_fixtures_po{
    constructor(){
        ...
        this.livescoreFixturesTab = '.esm_kommende_kampe_li'
        this.eventStatus = '.wff_status_value.wff_capitalize_text_transform'
        this.leagueNum = 1
    }

    livescore_fixtures_exists(){
        //livescore 
        ...
        cy.get(this.livescoreFixturesTab).should('exist').click()
    ...
    livescore_fixtures_all_matches(){
        cy.wait(2000)
            ...
            if($accountBox.length > 0){
                ...
                for(let i=0; i<this.counter; i++){
                    ...
                    .find(this.eventStatus).eq(i).invoke("text").then((text)=>{
                            ...
                            //expect(filteredText).to.match(/[0-9]+:[0-9]+/)
    ...
    livescore_fixtures_all_matches_mobile(){
        cy.wait(2000)
...
export class livescore_fixtures_other_sports_po{
    constructor(){
        ...
        this.statusInTooltip = '.wff_multiple_content_container'
        this.counter = 3
        this.livescoreResultsWidget = '.wff_widget.wff_widget_FW348BD5470C4DB389.wff_widget_wide'
        this.livescoreResultsMobile = '.wff_widget.wff_widget_FW348BD5470C4DB389.wff_widget_small'
        this.leagueHeader = '.esc_template_title'
        this.livescoreFixturesTab = '.esm_kommende_kampe_li'
        this.eventStatus = '.wff_status_value.wff_capitalize_text_transform'
        this.scopeHeader = '.wff_period_heading'
        this.scopeHeaderArray = ['1P', '2P', '3P', 'S']
        this.scopeHeaderMobileArray = ['S']
        this.leagueNum = 1
    }
    ...
    livescore_fixtures_all_matches(){
        ...
        cy.wrap(null).then(() => {
            ...
            if($accountBox.length > 0){
                ...
                //check the event statuses
                cy.get(this.livescoreResultsWidget).find(this.eventStatus).each((element, index) => {
    ...
    livescore_fixtures_all_matches_mobile(){
        ...
        cy.wrap(null).then(() => {
            ...
            if($accountBox.length > 0){
                ...
                //check the event statuses
                cy.get(this.livescoreResultsMobile).find(this.eventStatus).each((element, index) => {
...
export class team_page_livescore_fixtures_po{
    constructor(){
        ...
        this.statusInTooltip = '.wff_multiple_content_container'
        this.counter = 10
        this.livescoreFixturesWidget = '.wff_widget.wff_widget_FW9905637D40C61F88.wff_widget_wide'
        this.livescoreFixturesMobile = '.wff_widget.wff_widget_FW9905637D40C61F88.wff_widget_small'
        this.leagueHeader = '.esc_template_title'
        this.livescoreFixturesTab = '.esm_kommende_kampe_li'
        this.eventStatus = '.wff_event_start_time_status_container.wff_responsive_text.wff_status_notstarted.wff_single'
        this.leagueNum = 1
    ...
    livescore_fixtures_exists(){
        ...
        cy.get(this.livescoreFixturesWidget).should('exist')
    }
    ...
    livescore_fixtures_all_matches(){
            ...
            // for(let i=0; i<this.counter; i++){
                cy.get(this.livescoreFixturesWidget).find(this.eventStatus).each((element, index) => {
                cy.get(element).invoke("text").then((text)=>{
                    const filteredText = text.trim()
                    //    let statusAndTournament = filteredText.split("\n")
                        
                    // expect(filteredText).to.match(/[0-9]+\.\s[A-Za-z]+\s+[0-9]+:[0-9]+/)
                    
                    const regexPatterns = [
                        /[0-9]+\.\s[A-Za-z]+\s+[0-9]+:[0-9]+/,          // 17:00 - not started event - soccer
                        /[A-Za-z]+/               // Postponed - soccer, basketball ,etc
                        // Add more regex patterns here if needed
                    ];
            ...
            cy.get(this.eventStatus)
                ...
                // //check team names
            cy.get(this.livescoreFixturesWidget).find(this.teamNames)
                    .should("have.css","font-size","14px")
                    .and("have.css","font-style","normal")
                    .and("have.css","color","rgb(0, 0, 0)")
    ...
    livescore_fixtures_all_matches_mobile(){
                    ...
                    cy.get(this.eventStatus)
                        ...
                        // //check team names
                    cy.get(this.livescoreFixturesMobile).find(this.teamNames)
                            .should("have.css","font-size","14px")
                            .and("have.css","font-style","normal")
                            .and("have.css","color","rgb(0, 0, 0)")
