export class pitch_quick_po{
    constructor(){
        this.medium_widget = ".wff_widget.wff_widget_FW0A6E0338286A9DA3.wff_widget_medium"
    ...
    check_slider(){
        ...
        cy.get(this.medium_widget)
        .find(this.first_offensive_stat).should("have.attr","aria-label","Hjørnespark")
        //check the text in the aria-label
        cy.get(this.medium_widget)
        .find(this.first_offensive_stat).invoke("text").then((text)=>{
            const cut_text = text.trim()
            cy.wrap(cut_text).should("equal","Hjørnespark")
        })
        //click the slider
        cy.get(this.medium_widget)
    ...
    check_avrg_data(){
        //defensive
        cy.textNotEqualTo(this.medium_widget,this.average_value,"Pr. kamp 0.00")  
        //switch to offensive
        cy.get(this.slider_button)
        .click()
        //offensive
