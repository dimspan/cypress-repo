export class h2h_po{
    constructor(){
        this.h2h_code = ".wff_widget.wff_widget_FW2967B27C69586BBC.wff_widget_wide"
        this.headers = ".wff_h2h_section_data_header"
        this.flags = ".wff_flag_logo_img"
        this.form_icons_draw= ".wff_stats_ball_container.wff_circle_value_d"
        ...
        this.score_boxes = ".wff_event_result_single_row_link"
    ...
    check_visual_elements(){
        cy.get(this.h2h_code).within(()=>{
            //check flags
            cy.widthHeight(this.flags,"18px","18px")
            cy.get(this.flags)
            ...
            //if doesn't work this is why I removed it.
            ...
            cy.get(".wff_participant_link").eq(0).should("have.css","font","700 14px Roboto")
            .and("have.css","font-weight","700")
        })
    }
    check_score_boxes(){
        cy.get(this.h2h_code).within(()=>{
            cy.Height_Width_Border(this.score_boxes,"26px","60px","0px none rgb(0, 0, 0)")
            ...
            //check penalty score
            cy.penaltyScoreMultiple(this.penalty_score_home,"10px","14px","14px","50%","1px solid rgb(201, 201, 201)","center","center","absolute","-4px","-4px")
            cy.penaltyScoreMultiple(this.penalty_score_away,"10px","14px","14px","50%","1px solid rgb(201, 201, 201)","center","center","absolute","50px","-4px")
    ...
    check_status_and_date(){
        cy.get(this.h2h_code).within(()=>{
            cy.get(this.status_and_date_wrapper).invoke("css","margin-right").then((margin)=>{
                let rounded_margin = Math.round(margin.replace("px",""))
                cy.wrap(rounded_margin+"px").should("eq","950px")
            })
            // .should("have.css","margin-right","951.078px")
            ...
            cy.get(this.event_status).each((element,index)=>{
                cy.wrap(element).invoke("text").then((status_text)=>{
                    ...
                    const expectedValues = ["SES","SLUT","SET"]
    ...
    check_collapsing_functionality(){
        cy.get(this.h2h_code).within(()=>{
            ...
            cy.get(this.section_header).eq(2).click()
