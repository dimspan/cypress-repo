export class listPage_po{
    ...
    checks_ListPage_Design_fontSize(){
        ...
        try{
            cy.get(this.allLeagues).each((element,index)=>{
        ...
        try{
            ...
            .each((element,index)=>{
                cy.wrap(element).then(($element)=>{
                    if($element.is(":visible")){
                        cy.wrap($element).invoke("outerWidth").should("eq",20)  //checks the outerWidth
                        cy.wrap($element).invoke("outerHeight").should("eq",20) //checks the outerHeight
                    }else{
    ...
    checks_ListPage_Design_Mobile(){
        try{
            ...
            //check the flags 
            cy.get(this.flags).each((element,index)=>{
                cy.wrap(element).invoke("outerWidth").should("eq",20)  //checks the outerWidth
                cy.wrap(element).invoke("outerHeight").should("eq",20)
            })        
        }
        catch(error){
            console.error("An error occurred:", error.message)
    ...
    livescoresNextMatchesPC(){
        try{
            cy.getElement("css",this.nextMatchesSmall)
        }
        catch(error){
            console.error("An error occurred:", error.message)
            throw Error
