...
describe('[IH]Test the design and functionalities of EG iceHockeyMatchScore ',
{
    retries: {
      runMode: 0,
      openMode: 0,
...
() =>{
    ...
    beforeEach(()=>{
        ...
        po.selectedTemplate = ' EG Ice Hockey Match Score '
        po.firstPlayerName = po.secondPlayerName
        po.firstTeam = '#team_title_1'
        po.secondTeam = '#team_title_2'

        datapo.homeTeamName = '#team_title_1'
        datapo.awayTeamName = '#team_title_2'
        datapo.homePartExpTranslationText = 'НХЛ'
        datapo.typedText = ["NHL (USA)","2023/2024","Bulgarian","dd.mm.yy"]
        datapo.homeParticipantName = '#league'

        evpo.elArray = ['.ep_ig_edditable_component:eq(0)','.ep_ig_edditable_component:eq(1)','.ep_ig_edditable_component:eq(2)',
        '.ep_ig_edditable_component:eq(3)','.ep_ig_edditable_component:eq(4)','.ep_ig_edditable_component:eq(5)']
        evpo.highlightedEditableAreasCount = 6
        evpo.editableElementsCount = 6

        datapo.typedText = ["NHL (USA)","2023/2024","Bulgarian","dd.mm.yy"]

        context('[IH]PC view tests Match score', () =>{    
            ...
            it('[IH]C2 - Get to the EG Ice Hockey Match Score template',()=>{
                po.openExactGraphic()
                // cy.wait(1000)
            ...
            it('[IH]C3 - Check the design of the texts in the selected template',()=>{
                po.iceHockeyMatchScoreTemplDesign()
            })
